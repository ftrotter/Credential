<?php
//Generated by buildORM from the cred:Specialitys

	

	class SpecialityBase extends BaseORM{ //which extends Eloquent...

		var $table = 'Specialitys';

		protected $guarded = array(); //everything can be added via auto-fill... override this in custom code...
		

        //autogenerated this function... 
        public function created_by_User()
        {
                return $this->belongs_to('User','created_by_user_id');
        }


        //autogenerated this function... 
        public function modified_by_User()
        {
                return $this->belongs_to('User','modified_by_user_id');
        }


        //autogenerated this function... 
        public function ProviderEducation_bunch()
        {
                return $this->hasMany('ProviderEducation',speciality_id);
               // return $this->hasOne('ProviderEducation',speciality_id);
        }


        //autogenerated this function... 
        public function ProviderSpeciality_bunch()
        {
                return $this->hasMany('ProviderSpeciality',speciality_id);
               // return $this->hasOne('ProviderSpeciality',speciality_id);
        }


	}//the end of class Speciality		
?>